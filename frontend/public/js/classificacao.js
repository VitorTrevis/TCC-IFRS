...
(async () => {
  ...
  try {
    ...
    document.title = `Classificação - ${campeonato.nome}`;

    if (!blocos.length) {
      conteudo.innerHTML = '<div class="vazio"><strong>Campeonato de mata-mata</strong>Este formato não tem tabela de pontos.</div>';
      return;
    }
    ...
    conteudo.innerHTML = blocos.map((b) => `
      <section class="cartao mb-3">
        <div class="cartao-cabecalho"><h2 class="h6 mb-0">${b.grupo ? `Grupo ${esc(b.grupo)}` : 'Classificação geral'}</h2></div>
